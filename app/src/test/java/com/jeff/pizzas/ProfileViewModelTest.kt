...
import com.jeff.pizzas.ui.profile.ProfileViewModel
import com.jeff.pizzas.utils.Event
import com.jeff.pizzas.utils.TestCoroutineRule
import com.jeff.pizzas.utils.dummy.userDummy1
...
class ProfileViewModelTest {
    ...
    @Test
    fun `update user to Repository correctly`() {
        eventGetUser = Event(Event.Status.SUCCESS, userDummy, null)
        liveDataEvent = MutableLiveData(eventGetUser)
        ...
        testCoroutineRule.runBlockingTest {
            ...
            doReturn(userDummy1)
                .`when`(repository)
                .updateUser(userDummy1)

            val viewModel = ProfileViewModel(repository)
            viewModel.updateEvent.observeForever(userObserver)
            viewModel.updateUser(userDummy1)

            verify(repository).updateUser(userDummy1)
            verify(userObserver).onChanged(Event.success(userDummy1))

            assertEquals(userDummy1, viewModel.updateEvent.value?.data)

            viewModel.updateEvent.removeObserver(userObserver)
        }
    }
